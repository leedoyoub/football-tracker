...
import { CombinationRecords } from '../components/CombinationRecords'
...
type Category = 'player' | 'combination' | 'team' | 'history' | 'insights' | 'integrity'
...
export function RecordsScreen({ season, onNavigate }: { season: string; onNavigate: (view: View) => void }) {
  ...
  const groups = category === 'player' ? playerGroups : teamGroups
  ...
  // Explicit advanced tab only; the historical scan never runs on normal navigation.
  const integrity = useMemo(() => category === 'integrity' ? auditDataIntegrity(matches, players, teams) : null, [category, matches, players, teams])
  ...
  return <div className="px-4 pb-8 pt-6"><div className="mb-4 flex items-center justify-between"><h1 className="text-2xl font-semibold">Records</h1>{!['history', 'insights', 'integrity'].includes(category) && <RankingFilterButton applied={filters} onApply={setFilters} seasons={availableSeasons} teams={teams} />}</div><div className="no-scrollbar mb-5 flex gap-1 overflow-x-auto rounded-xl bg-zinc-900 p-1">{([['player', 'Player'], ['combination', 'Combination'], ['team', 'Team'], ['history', 'History'], ['insights', 'Insights'], ['integrity', 'Data Integrity']] as [Category, string][]).map(([id, label]) => <button key={id} type="button" onClick={() => { setCategory(id); setExpanded(null) }} className={`shrink-0 rounded-lg px-3 py-2 text-[10px] font-bold ${category === id ? 'bg-emerald-500 text-black' : 'text-zinc-400'}`}>{label}</button>)}</div>
    {category !== 'history' && category !== 'insights' && category !== 'integrity' && category !== 'combination' && groups.map(renderGroup)}
    {category === 'combination' && <CombinationRecords players={players} matches={scoped} />}
