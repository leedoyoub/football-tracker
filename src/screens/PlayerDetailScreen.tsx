...
export function PlayerDetailScreen({ playerId, season, onNavigate }: { playerId: string; season: string; onNavigate: (view: View) => void }) {
  ...
  const statsBase = player ? playerSeasonStats(player, matches, active) : null
  ...
  const records = matches.filter((match) => match.season === active && match.appearances.some((appearance) => appearance.playerId === player.id && selectedTeamIds.includes(appearance.teamId))).sort((a, b) => b.matchDay - a.matchDay || b.date.localeCompare(a.date))
  ...
  const average = rated.length ? rated.reduce((total, item) => total + item.rating.rating, 0) / rated.length : 0
  ...
  return <div className="px-4 pb-8 pt-6">
    ...
    <div className="mb-3 grid grid-cols-4 gap-2">{[['Avg', average ? average.toFixed(1) : '—'], ['Apps', String(rated.length)], ['G', String(goals)], ['A', String(assists)]].map(([label, value]) => <div key={label} className="rounded-2xl bg-zinc-900 px-2 py-3 text-center"><div className="text-lg font-bold">{value}</div><div className="text-[10px] uppercase tracking-wide text-zinc-500">{label}</div></div>)}</div>
    <div className="mb-5 grid grid-cols-4 gap-2">{[['Starts', starts], ['Subs', subApps], ['Min', minutes], ['MOM', mom], ['G/90', minutes ? (goals / minutes * 90).toFixed(2) : '—'], ['A/90', minutes ? (assists / minutes * 90).toFixed(2) : '—'], ['G+A/90', minutes ? ((goals + assists) / minutes * 90).toFixed(2) : '—'], ['W-D-L', `${results.wins}-${results.draws}-${results.losses}`]].map(([label, value]) => <div key={String(label)} className="rounded-xl bg-zinc-900 px-1 py-2 text-center"><div className="text-sm font-bold">{value}</div><div className="text-[9px] uppercase tracking-wide text-zinc-500">{label}</div></div>)}</div>
    ...
    <div className="space-y-3">
      ...
      {records.map((match) => {
        ...
        const isMom = match.manOfMatchPlayerId === player.id
