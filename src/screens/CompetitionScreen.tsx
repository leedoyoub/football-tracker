...
function LeagueView({ season, teams, players, matches, league, onNavigate }: { season: string; teams: Team[]; players: Player[]; matches: Match[]; league: ReturnType<typeof leagueCompetition>; onNavigate: (view: View) => void }) {
  const [playerMetric, setPlayerMetric] = useState<RankingDisplayMetric>('rating')
...
function BestEleven({ title, xi, bestPlayerId, players, teams, onNavigate }: { title: string; xi: ReturnType<typeof unifiedBestEleven>; bestPlayerId?: string; players: Player[]; teams: Team[]; onNavigate: (view: View) => void }) {
  return <AwardBestXI title={title} result={{ bestXI: xi.slots, statsByPlayer: xi.statsByPlayer, bestPlayerId }} players={players} teams={teams} open={title === 'Team of the Season'} onPlayerOpen={id => onNavigate({ name: 'player', id })} />
}
