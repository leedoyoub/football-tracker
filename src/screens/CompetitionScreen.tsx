...
const leagueViewMemory = new Map<string, { tab: 'players' | 'table' | 'form' | 'history'; day: number; compared: string[] }>()
...
function DeferredCompetitionPanels({ season, type, initialMetric, players, teams, tournamentTeams, matches, leagueMatches, draw, cup, champions, finalized, nextSeason, onComplete, onNavigate }: { season: string; type: CompetitionType; initialMetric?: 'rating' | 'goals' | 'assists' | 'mom'; players: Player[]; teams: Team[]; tournamentTeams: Team[]; matches: Match[]; leagueMatches?: Match[]; draw?: CompetitionState; cup: CupCompetition | null; champions: ReturnType<typeof championsCompetition> | null; finalized: boolean; nextSeason: string; onComplete: () => void; onNavigate: (view: View) => void }) {
  ...
  return <>
    ...
    {type !== 'league' && stage >= 1 && <DeferredCompetitionRankings season={season} type={type} initialMetric={initialMetric} players={players} teams={teams} allMatches={matches} leagueMatches={leagueMatches} onNavigate={onNavigate} />}
...
function LeagueView({ season, teams, players, matches, league, onNavigate }: { season: string; teams: Team[]; players: Player[]; matches: Match[]; league: ReturnType<typeof leagueCompetition>; onNavigate: (view: View) => void }) {
  const [playerMetric, setPlayerMetric] = useState<'rating' | 'goals' | 'assists' | 'mom'>('rating')
  const [bestXiMode, setBestXiMode] = useState<'season' | 'monthly'>('season')
  const [tab, setTab] = useState<'players' | 'table' | 'form' | 'history'>(() => leagueViewMemory.get(season)?.tab ?? 'table')
  ...
  const playerRows = useMemo(() => rankGlobalRankingRows(buildGlobalRankingData(players, analytics.leagueMatches, { seasons: [season], teams: [], positions: [] }, 'rating'), players, playerMetric).slice(0, 7), [players, analytics.leagueMatches, season, playerMetric])
  const playerById = useMemo(() => new Map(players.map(player => [player.id, player])), [players])
  const playerValue = (row: typeof playerRows[number]) => playerMetric === 'rating' ? row.avgRating.toFixed(2) : String(row[playerMetric])
  const seasonXi = useMemo(() => unifiedBestEleven(players, analytics.leagueMatches, season), [players, analytics.leagueMatches, season])
  return <section><CompetitionHeader type="league" label="League" season={season} champion={league.championId ? teams.find(team => team.id === league.championId) : undefined} /><div className="mb-4 flex items-center justify-between rounded-xl bg-zinc-900 px-3 py-2.5"><span className="text-xs text-zinc-400">League progress</span><b className="text-sm text-emerald-400">{league.complete ? 'Completed · 30/30' : `MD ${league.matchdayProgress} / ${LEAGUE_MATCHES_PER_TEAM}`}</b></div><SegmentedControl label="League view" value={tab} onChange={setTab} options={[{ value: 'players', label: 'Players' }, { value: 'table', label: 'Table' }, { value: 'form', label: 'Form' }, { value: 'history', label: 'History' }]} />
    <div className="mt-3">{tab === 'players' && <><SegmentedControl label="League player category" value={playerMetric} onChange={value => setPlayerMetric(value as typeof playerMetric)} options={[{ value: 'rating', label: 'Rating' }, { value: 'goals', label: 'Goals' }, { value: 'assists', label: 'Assists' }, { value: 'mom', label: 'MOM' }]} /><section className="mt-3"><SectionHeader title="Global Ranking" subtitle="Top 7" /><div className="mt-2 overflow-hidden rounded-xl bg-zinc-900">{playerRows.map((row, index) => <button key={row.playerId} type="button" onClick={() => onNavigate({ name: 'player', id: row.playerId })} className="flex min-h-11 w-full items-center gap-2 border-b border-white/5 px-3 text-left last:border-0"><b className="w-5 text-xs text-zinc-500">#{index + 1}</b><span className="min-w-0 flex-1 truncate text-xs font-bold">{playerFullName(playerById.get(row.playerId))}</span><b className="text-xs text-emerald-300">{playerValue(row)}</b></button>)}</div><button type="button" onClick={() => onNavigate({ name: 'global-ranking', season, competitionType: 'league', rankingMetric: playerMetric })} className="secondary-view-all mt-3 w-full">View All</button></section>{['goals', 'assists', 'mom', 'rating'].includes(playerMetric) && <RaceHistoryPanel analytics={analytics} metric={playerMetric} players={Object.fromEntries(players.map(player => [player.id, player]))} />}<section className="mt-5"><SegmentedControl label="League Best XI" value={bestXiMode} onChange={value => setBestXiMode(value as typeof bestXiMode)} options={[{ value: 'season', label: 'Season Best XI' }, { value: 'monthly', label: 'Team of the Month' }]} />{bestXiMode === 'season' ? <BestEleven title="Season Best XI" xi={seasonXi} players={players} teams={teams} onNavigate={onNavigate} /> : analytics.latestMonthlyAwards ? <><p className="mt-2 text-[10px] text-zinc-500">Team of the Month · {season}-{analytics.latestMonthlyAwards.block.id}. Player of the Month is marked blue.</p><Pitch slots={analytics.latestMonthlyAwards.bestXI} players={players} teams={teams} statsByPlayer={analytics.latestMonthlyAwards.statsByPlayer} presentation="history" showPositionBadge={false} onSlotClick={slot => slot.playerId && onNavigate({ name: 'player', id: slot.playerId })} /></> : <Empty text="Not available yet." />}</section></>}{tab === 'table' && (current.length ? <StandingsTable standings={current} teams={teams} compact onTeamNavigate={id => onNavigate({ name: 'team', id })} /> : <Empty text="No League data for this season." />)}
