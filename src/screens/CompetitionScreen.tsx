...
function LeagueView({ season, teams, players, matches, league, onNavigate }: { season: string; teams: Team[]; players: Player[]; matches: Match[]; league: ReturnType<typeof leagueCompetition>; onNavigate: (view: View) => void }) {
  ...
  const playerMetrics = ['rating', 'goals', 'assists', 'mom'] as const
  ...
  const playerSwipe = useMetricSwipe(playerMetrics, playerMetric, setPlayerMetric)
...
function CompetitionRankings({ season, type, initialMetric, players, teams, matches, onNavigate }: { season: string; type: CompetitionType; initialMetric?: 'rating' | 'goals' | 'assists' | 'mom'; players: Player[]; teams: Team[]; matches: Match[]; onNavigate: (view: View) => void }) {
  ...
  const rankingSwipe = useMetricSwipe(METRICS.map(item => item.id), metric, next => { setMetric(next); setAll(false) })
