...
import { leagueCompetition } from '../engine/competition'
...
export function StandingsScreen({ season, onNavigate }: { season: string; onNavigate: (view: View) => void }) {
  ...
  const analytics = buildSeasonAnalytics(teams, players, matches, season)
  const standings = analytics.leagueSnapshots.get(analytics.currentMatchDay)?.standings ?? leagueCompetition(teams, matches, season, players).standings
