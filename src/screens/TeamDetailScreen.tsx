...
function TeamBestPlayers({ team, teamId, selectedSeason, competition, seasons, players, matches, onSeason, onCompetition, onPlayer, onViewAll }: { team: Team; teamId: string; selectedSeason: string; competition: CompetitionType | 'all'; seasons: string[]; players: Parameters<typeof buildGlobalRankingData>[0]; matches: Parameters<typeof buildGlobalRankingData>[1]; onSeason: (season: string) => void; onCompetition: (competition: CompetitionType | 'all') => void; onPlayer: (id: string) => void; onViewAll: (metric: 'rating' | 'goals' | 'assists' | 'mom') => void }) {
  ...
  const metrics = ['rating', 'goals', 'assists', 'mom'] as const
  ...
  const swipe = useMetricSwipe(metrics, metric, setMetric)
  return <section className="mb-6"><div className="mb-3 flex items-center justify-between"><h2 className="text-lg font-semibold">Best Players</h2><CompetitionScopeSelector value={competition} onChange={onCompetition} /></div><div className="mb-3"><select aria-label="Best Players season" value={selectedSeason} onChange={event => onSeason(event.target.value)} className="w-full rounded-xl bg-zinc-900 px-3 py-2 text-xs font-bold">{seasons.map(item => <option key={item}>{item}</option>)}</select></div><SegmentedControl label="Best Players category" value={metric} onChange={value => setMetric(value as typeof metric)} options={metrics.map(value => ({ value, label: value === 'mom' ? 'MOM' : value[0].toUpperCase() + value.slice(1) }))} /><div {...swipe} className="mt-2 overflow-hidden rounded-xl bg-zinc-900 touch-pan-y" aria-label="Swipe Team Best Player metrics">{rows.map((row, index) => { const player = byId.get(row.playerId); return <div key={row.playerId} className="border-b border-white/5 last:border-0"><RankingRow rank={index + 1} player={player} team={team} value={value(row)} onClick={() => player && onPlayer(player.id)} /></div> })}{!rows.length && <p className="p-3 text-xs text-zinc-500">No qualifying players yet.</p>}</div><button type="button" onClick={() => onViewAll(metric)} className="secondary-view-all mt-3 w-full">View All</button></section>
}
