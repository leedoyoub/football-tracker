...
function TeamBestPlayers({ teamId, selectedSeason, competition, seasons, players, matches, onSeason, onCompetition, onPlayer, onViewAll }: { teamId: string; selectedSeason: string; competition: CompetitionType | 'all'; seasons: string[]; players: Parameters<typeof buildGlobalRankingData>[0]; matches: Parameters<typeof buildGlobalRankingData>[1]; onSeason: (season: string) => void; onCompetition: (competition: CompetitionType | 'all') => void; onPlayer: (id: string) => void; onViewAll: (metric: 'rating' | 'goals' | 'assists' | 'mom') => void }) {
  const [metric, setMetric] = useState<'rating' | 'goals' | 'assists' | 'mom'>('rating')
  ...
  const value = (row: typeof rows[number]) => metric === 'rating' ? row.avgRating.toFixed(2) : String(row[metric])
  return <section className="mb-6"><div className="mb-3 flex items-center justify-between"><h2 className="text-lg font-semibold">Best Players</h2><CompetitionScopeSelector value={competition} onChange={onCompetition} /></div><div className="mb-3"><select aria-label="Best Players season" value={selectedSeason} onChange={event => onSeason(event.target.value)} className="w-full rounded-xl bg-zinc-900 px-3 py-2 text-xs font-bold">{seasons.map(item => <option key={item}>{item}</option>)}</select></div><SegmentedControl label="Best Players category" value={metric} onChange={value => setMetric(value as typeof metric)} options={[{ value: 'rating', label: 'Rating' }, { value: 'goals', label: 'Goals' }, { value: 'assists', label: 'Assists' }, { value: 'mom', label: 'MOM' }]} /><div className="mt-2 space-y-2">{rows.map((row, index) => { const player = byId.get(row.playerId); return <button key={row.playerId} type="button" disabled={!player} onClick={() => player && onPlayer(player.id)} className="flex min-h-12 w-full items-center gap-2 rounded-xl bg-zinc-900 px-3 py-2 text-left disabled:opacity-50"><span className="w-7 text-xs font-black text-zinc-500">#{index + 1}</span><PlayerAvatar photoUrl={player?.photoUrl || player?.image} number={player?.number} className="h-9 w-9 text-[9px]" /><span className="min-w-0 flex-1"><b className="block truncate text-xs">{player ? playerFullName(player) : 'No qualifying player'}</b></span><strong className="text-sm text-emerald-300">{value(row)}</strong></button> })}</div><button type="button" onClick={() => onViewAll(metric)} className="secondary-view-all mt-3 w-full">View All</button></section>
}
