...
export function HomeScreen({ season, onNavigate }: { season: string; onSeason?: (season: string) => void; onNavigate: (view: View) => void }) {
  ...
  const leaderRows = rankings.get(metric) ?? []
  ...
  return <div className="px-4 pb-8 pt-6">
    ...
    <section className="mb-6" data-home-section="recent-matches"><SectionHeader title="Recent Matches" action={matches.length > 5 ? <button type="button" onClick={() => onNavigate({ name: 'results' })} className="secondary-view-all">View All</button> : undefined} /><div className="mt-2 grid grid-cols-5 gap-1.5" aria-label="Latest five recent matches">{recent.length ? recent.map(result => { const competition = result.match.competitionType === 'champions' ? 'Champions' : result.match.competitionType === 'cup' ? 'Cup' : 'League'; const tone = result.outcome === 'W' ? 'border-emerald-300/50 bg-emerald-500/20' : result.outcome === 'L' ? 'border-red-300/50 bg-red-500/20' : 'border-zinc-400/30 bg-zinc-700/50'; return <button key={result.match.id} type="button" onClick={() => onNavigate({ name: 'match', id: result.match.id })} aria-label={`${competition}, ${result.outcome === 'W' ? 'win' : result.outcome === 'L' ? 'loss' : 'draw'}, ${result.goalsFor} to ${result.goalsAgainst}`} className={`flex min-h-20 min-w-0 flex-col items-center justify-between rounded-xl border px-1 py-2 text-center ${tone}`}><span className="text-[8px] font-black tracking-wide text-zinc-300">{competition}</span><TeamIcon team={teamById[result.teamId]} className="h-7 w-7 text-[7px]" /><b className="text-xs tabular-nums">{result.goalsFor}–{result.goalsAgainst}</b></button> }) : <div className="col-span-5"><Empty text="No completed matches yet." /></div>}</div></section>
