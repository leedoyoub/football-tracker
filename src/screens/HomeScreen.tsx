...
export function HomeScreen({ season, onNavigate }: { season: string; onSeason?: (season: string) => void; onNavigate: (view: View) => void }) {
  ...
  const leaders = useMemo(() => {
    ...
    return [
      { label: 'Top Scorer', row: eligible.slice().sort((a, b) => b.goals - a.goals || b.avgRating - a.avgRating)[0], value: (row: typeof eligible[number]) => `${row.goals} G` },
      { label: 'Top Assists', row: eligible.slice().sort((a, b) => b.assists - a.assists || b.avgRating - a.avgRating)[0], value: (row: typeof eligible[number]) => `${row.assists} A` },
      { label: 'Best Rated', row: eligible.slice().sort((a, b) => b.avgRating - a.avgRating || b.matches - a.matches)[0], value: (row: typeof eligible[number]) => row.avgRating.toFixed(2) },
    ]
  }, [seasonStats])
  ...
  const progress: { type: CompetitionType; label: string; value: string }[] = [
    { type: 'league', label: 'League', value: league.complete ? 'Completed' : `MD ${league.matchdayProgress} / 38` },
  ...
  return <div className="px-4 pb-8 pt-6">
    ...
    <section className="mb-6" data-home-section="competition-progress"><h2 className="mb-2 text-lg font-semibold">Competition Progress</h2><div className="grid grid-cols-3 gap-2">{progress.map(item => <button key={item.type} type="button" onClick={() => onNavigate({ name: 'competition', season, competitionType: item.type })} className="min-h-20 rounded-xl border border-white/5 bg-zinc-900 p-2.5 text-left"><b className="block text-xs">{item.label}</b><span className="mt-2 block text-[10px] leading-tight text-zinc-400">{item.value}</span></button>)}</div></section>
