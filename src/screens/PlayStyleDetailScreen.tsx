...
export function PlayStyleDetailScreen({ style, onNavigate, onBack }: { style: TeamPlayStyle; onNavigate: (view: View) => void; onBack: () => void }) {
  ...
  const teamRows = useMemo(() => trackedStyleTeams(teams, style).map(team => {
    ...
    const totals = games.reduce((value, match) => { 
        const result = trackedTeamResult(match, team.id);
        value[trackedTeamResult(match, team.id).outcome]++;
        value.GF += result.goalsFor;
        value.GA += result.goalsAgainst;
        return value 
