...
export function MatchDetailScreen({ matchId, onNavigate }: { matchId: string; onNavigate: (view: View) => void }) {
  ...
  return <div className="px-4 pb-8 pt-6">
    ...
    <section className="mt-6 rounded-2xl bg-zinc-900 p-3"><h2 className="text-sm font-semibold">Player minutes</h2><p className="mt-1 text-[10px] text-zinc-500">Official regulation-time minutes; stoppage-time events remain in the timeline.</p><div className="mt-3 grid grid-cols-2 gap-1.5 text-xs">{[...starters, ...bench].map(appearance => <div key={appearance.playerId} className="flex items-center justify-between rounded-lg bg-black/20 px-2 py-1.5"><span className="truncate pr-2">{byId[appearance.playerId] ? byId[appearance.playerId].displayName ?? byId[appearance.playerId].name : appearance.playerId}</span><b>{ratings[appearance.playerId] ? `${ratings[appearance.playerId].minutes}'` : 'Unused'}</b></div>)}</div></section>
