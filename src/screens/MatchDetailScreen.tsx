...
function MatchChangesPanel({ matchId, players, teams, matches, competitionStates }: { matchId: string; players: Player[]; teams: Team[]; matches: ReturnType<typeof useStore>['matches']; competitionStates: CompetitionState[] }) {
  ...
  return <details className="mb-4 rounded-xl border border-emerald-500/20 bg-emerald-500/10 p-3" onToggle={event => setOpen(event.currentTarget.open)}>
    ...
    {open && <div className="mt-2 space-y-2">{changes.map(change => <article key={change.id} className={`rounded-lg border p-2 ${change.fallback ? 'border-white/10 bg-black/20' : 'border-emerald-400/15 bg-black/10'}`}><div className="flex items-center justify-between gap-2"><b className="truncate text-xs">{change.title}</b><span className={`shrink-0 rounded-full px-1.5 py-0.5 text-[8px] font-black uppercase ${change.category === 'takeover' || change.category === 'rare' ? 'bg-emerald-400 text-black' : 'bg-white/10 text-zinc-300'}`}>{change.category === 'takeover' ? 'Takes #1' : change.category.replace('-', ' ')}</span></div><p className="mt-1 text-xs font-black text-emerald-200">{change.primary}</p>{change.secondary.map((detail, index) => <p key={`${change.id}:${index}`} className="mt-1 text-[10px] text-zinc-400">{detail}</p>)}</article>)}</div>}
  </details>
}
