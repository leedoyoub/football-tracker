...
export function MatchDetailScreen({ matchId, onNavigate }: { matchId: string; onNavigate: (view: View) => void }) {
  ...
  const slots: Best11Slot[] = kickoff.map((slot) => ({ slot: slot.id, position: slot.ratingPosition ?? slot.matchPosition, matchPosition: slot.ratingPosition ?? slot.matchPosition, playerId: slot.playerId, teamId, avgRating: ratings[slot.playerId ?? '']?.rating ?? 0, matches: ratings[slot.playerId ?? ''] ? 1 : 0, x: slot.x, y: slot.y }))
