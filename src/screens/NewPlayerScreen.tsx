...
export function NewPlayerScreen({ teamId, onReplace, onBack }: { teamId?: string; onReplace: (view: View) => void; onBack: () => void }) {
  ...
  const save = () => { try { if (selectedPlayer) { updatePlayer(selectedPlayer.id, { teamIds: [...new Set([selectedPlayer.teamId, ...(selectedPlayer.teamIds ?? []), selectedTeam].filter(Boolean))], ...(apiSelected ? { externalPlayerId: apiSelected.id, photoUrl: apiSelected.photo || selectedPlayer.photoUrl } : {}) }); onReplace({ name: 'player', id: selectedPlayer.id }); return }; const id = addPlayer({ name: displayName.trim(), fullName: fullName.trim() || displayName.trim(), displayName: displayName.trim(), number, position, teamId: selectedTeam, ...(apiSelected ? { externalPlayerId: apiSelected.id, photoUrl: apiSelected.photo || undefined } : {}) }); onReplace({ name: 'player', id }) } catch (error) { setSaveError(error instanceof RosterCapacityError ? 'That team is full (23/23). Choose another team or No Team.' : 'Could not save player.') } }
  return <div className="px-4 pb-8 pt-6"><button type="button" onClick={onBack} className="mb-3 text-xs font-semibold text-emerald-400">← Cancel</button><h1 className="mb-4 text-2xl font-semibold">New player</h1>
