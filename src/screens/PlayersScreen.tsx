  ...
  const rows = useMemo(() => {
    ...
    return players
      ...
      .map(player => {
        ...
        return { player, team: teams.find(team => team.id === player.teamId), matchesFilter: (!appliedFilters.seasons.length && !appliedFilters.teams.length) || playerMatches.length > 0 || (appliedFilters.teams.includes('__no-team__') && playerHasNoCurrentTeam(player)) || (!appliedFilters.seasons.length && [player.teamId, ...(player.teamIds ?? [])].some(id => appliedFilters.teams.includes(id))) }
      })
