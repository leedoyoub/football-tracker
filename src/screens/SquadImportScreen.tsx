...
export function SquadImportScreen({ teamId, onBack }: { teamId: string; onBack: () => void }) {
  ...
  const loadSquad = async (forceRefresh = false) => {
    ...
    try {
      const result = await fetchApiFootballSquad(team.externalTeamId, { forceRefresh })
      setSquad(result)
      setSelected(new Set())
  ...
  return <div className="px-4 pb-8 pt-6">
    ...
    <div className="mb-5 flex items-start justify-between gap-3"><div><h1 className="text-2xl font-semibold">Import squad</h1><p className="mt-1 text-xs text-zinc-400">{team.name} · API-Football</p></div><button type="button" onClick={() => void loadSquad(true)} disabled={loading || !user || !team.externalTeamId} className="rounded-full bg-zinc-800 px-3 py-2 text-xs font-bold disabled:opacity-40">Retry</button></div>
