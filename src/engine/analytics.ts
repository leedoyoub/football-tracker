...
export type CombinationStats = {
  ...
  startingGoalsFor: number; startingGoalsAgainst: number; startingCombinedGA: number; startingGoalInvolvements: number; startingOpponentSot: number; startingWins: number; startingDraws: number; startingLosses: number; startingCleanSheets: number
...
export function combinationStats(players: Player[], matches: Match[], filter: AnalyticsFilter, kind: CombinationKind): CombinationStats[] {
  ...
  for (const match of matches.filter(match => isScoped(match, filter))) for (const teamId of teamIds(match)) {
    ...
    const add = (ids: string[], roles: Record<string, RoleCheck>) => {
      ...
      previous.togetherMinutes += entry.togetherMinutes; previous.matches += entry.matches; previous.startsTogether += entry.startsTogether; previous.goalsFor += entry.goalsFor; previous.goalsAgainst += entry.goalsAgainst; previous.weightedOpponentSot += entry.weightedOpponentSot; previous.onPitchGoalsFor += entry.onPitchGoalsFor; previous.onPitchGoalsAgainst += entry.onPitchGoalsAgainst; previous.onPitchGoalDifference += entry.onPitchGoalDifference; previous.startingGoalsFor += entry.startingGoalsFor; previous.startingGoalsAgainst += entry.startingGoalsAgainst; previous.startingCombinedGA += entry.startingCombinedGA; previous.startingGoalInvolvements += entry.startingGoalInvolvements; previous.startingOpponentSot += entry.startingOpponentSot; previous.startingWins += entry.startingWins; previous.startingDraws += entry.startingDraws; previous.startingLosses += entry.startingLosses; previous.startingCleanSheets += entry.startingCleanSheets; previous.goalDifference += entry.goalDifference; previous.combinedGoals += entry.combinedGoals; previous.combinedAssists += entry.combinedAssists; previous.combinedGA += entry.combinedGA; previous.averageRating = (previous.averageRating * (previous.matches - 1) + entry.averageRating) / previous.matches; previous.wins += entry.wins; previous.draws += entry.draws; previous.losses += entry.losses; previous.cleanSheets += entry.cleanSheets
    }
...
export function goalPartnerships(players: Player[], matches: Match[], filter: AnalyticsFilter): GoalPartnership[] {
  ...
  for (const match of matches.filter(match => isScoped(match, filter))) for (const event of match.events) {
    ...
    const scorerAppearance = match.appearances.find(item => item.playerId === event.playerId && item.teamId === teamId)
    const assisterAppearance = match.appearances.find(item => item.playerId === event.assistPlayerId && item.teamId === teamId)
    if (!scorerAppearance || !assisterAppearance || !isOnPitchAtEvent(match, scorerAppearance, event) || !isOnPitchAtEvent(match, assisterAppearance, event)) continue
