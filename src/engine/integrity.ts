...
const cache = new WeakMap<Match[], WeakMap<Player[], WeakMap<Team[], IntegrityReport>>>()
...
/** Read-only diagnostic pass. It neither normalizes nor repairs historical data. */
export function auditDataIntegrity(matches: Match[], players: Player[], teams: Team[]): IntegrityReport {
  ...
  const cached = byTeams.get(teams); if (cached) return cached
  ...
  byTeams.set(teams, report); return report
}
