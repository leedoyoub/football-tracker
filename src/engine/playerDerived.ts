...
const scoped = (matches: Match[], scope: PlayerScope) => matches.filter(match => (!scope.season || match.season === scope.season) && (!scope.competition || scope.competition === 'all' || matchCompetitionType(match) === scope.competition))
