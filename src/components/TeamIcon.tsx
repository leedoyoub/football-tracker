...
export function TeamIcon({ team, children, className = '' }: { team?: Team; children?: ReactNode; className?: string }) {
  ...
  return <span className={`relative flex shrink-0 items-center justify-center overflow-hidden rounded-full ${className}`} style={{ background: striped ? undefined : primary }}><span className="absolute inset-0 flex">{striped && <><i className="flex-1" style={{ background: primary }} /><i className="flex-1" style={{ background: secondary }} /><i className="flex-1" style={{ background: primary }} /></>}</span><span className="relative z-10">{children}</span></span>
}
