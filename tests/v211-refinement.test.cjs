...
test('v2.1.5 central version and focused Player Detail refinement are current', () => {
  const detail = fs.readFileSync(require.resolve('../src/screens/PlayerDetailScreen.tsx'), 'utf8')
  assert.equal(APP_VERSION, '2.1.5')
  assert.equal(require('../package.json').version, '2.1.5')
