...
test('v2.2.1 final home is compact and routes each consolidated detail destination', () => {
  const home = fs.readFileSync(require.resolve('../src/screens/HomeScreen.tsx'), 'utf8')
  for (const token of ['Season dashboard', 'Recent Matches', 'Highlights', 'Season Leaders', 'slice(0, 3)', 'slice(0, 5)', "kind: 'monthly'", "kind: 'review'", "kind: 'news'", 'rankingMetric']) assert(home.includes(token), token)
