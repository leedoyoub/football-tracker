...
test('v2.1.9 version is consistent and rating revision stays fixed', () => {
  assert.equal(APP_VERSION, '2.1.9')
  assert.equal(require('../package.json').version, '2.1.9')
  assert.equal(require('../package-lock.json').version, '2.1.9')
  assert.equal(RATING_ENGINE_REVISION, 5)
})
