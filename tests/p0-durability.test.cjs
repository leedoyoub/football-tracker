...
test('store serializes durable saves and restores the queue after a rejected write', () => {
  ...
  assert(store.includes('const operation = persistenceQueue.current.then(() => LocalRepository.saveAppState(next))'))
  assert(store.includes('persistenceQueue.current = operation.then(() => undefined, () => undefined)'))
})
...
const syncHarness = ({ initial, cloudTeams, meta = {}, queue = [] }) => {
  ...
  const priorStorage = global.localStorage; const priorNavigator = global.navigator
  const entries = { 'football-tracker-v1': JSON.stringify(initial) }; global.localStorage = memoryStorage(entries); global.navigator = { onLine: true }
  ...
  return { SyncManager, entries, stores, uploaded, restore: () => { Module._load = originalLoad; global.localStorage = priorStorage; global.navigator = priorNavigator } }
}
